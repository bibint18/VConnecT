...
const Header: React.FC = () => {
  ...
  const { name, isAuthenticated, accessToken } = useAppSelector(
    (state) => state.user
  );
  ...
  return (
    <header className="bg-white text-black p-4 flex justify-between items-center relative z-50">
      ...
      <nav className="hidden md:flex space-x-6">
        ...
        <Link to="/rooms" className="hover:text-purple-500">
          Rooms
        </Link>
        <Link to="/friends" className="hover:text-purple-500">
          Friends
        </Link>
        <Link to="/profile" className="hover:text-purple-500">
          Profile
        </Link>
        <Link to="/dailyTrivia" className="hover:text-purple-500">
          Daily Trivia
        </Link>
        <Link to="/feed" className="hover:text-purple-500">
          Community
        </Link>
      </nav>
          ...
          <nav className="!flex !flex-col !p-4 !space-y-3">
            ...
            {isAuthenticated ? (
              <div className="!flex !flex-col !space-y-3 !mt-2">
                <span className="!text-gray-700 !font-medium !py-2 !px-3">
                  {name}
                </span>
                <button
                  onClick={() => {
