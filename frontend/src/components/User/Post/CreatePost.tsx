...
import { CommunityService, CloudinaryUploadResult } from '@/services/CommunityService';
  ...
  return (
    <div className="!flex !flex-col !gap-6 !w-full">
      <motion.div
        ...
        animate="visible"
      >
        <h2 className="!text-2xl !font-semibold !text-gray-900 !text-center !mb-4">Create New Post</h2>
        {error && <p className="!text-red-500 !text-center !text-base !mb-4">{error}</p>}
        <form onSubmit={handleFormSubmit} className="!flex !flex-col !gap-4">
          ...
          <div className="!flex !flex-col !gap-2">
            <label className="!text-sm !font-medium !text-gray-700">Media (Images/Videos)</label>
            <div className="!flex !items-center !border !border-gray-200 !rounded-lg !bg-gray-50 !overflow-hidden">
              <motion.button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                disabled={isUploading}
                className={`!p-3 !bg-white !border-r !border-gray-200 ${isUploading ? '!opacity-50 !cursor-not-allowed' : '!hover:bg-gray-100'}`}
              ...
              <input
                ...
                className="!hidden"
              />
              <span className="!flex-1 !p-3 !text-sm !text-gray-600">Select a file</span>
            </div>
          </div>
          <div className="!flex !justify-center">
            <motion.button
              type="submit"
              className={`!bg-blue-500 !text-white !px-6 !py-2 !rounded-lg !font-medium !hover:bg-blue-600 ${isUploading ? '!opacity-50 !cursor-not-allowed' : ''}`}
