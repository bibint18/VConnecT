...
import toast from 'react-hot-toast';
import { IUser } from '@/components/admin/dashboard/CustomerDashboard';
// Import Lucide React icons for lock, unlock, search, and delete
...
interface Room {
  ...
  type: 'PUBLIC' | 'PRIVATE';
  createdAt: string;
  createdBy: string; // Added to track the user who created the room
}
...
const ListRoom: React.FC = () => {
  ...
  // Handle room deletion with confirmation and toast notification
  const handleDeleteRoom = async (roomId: string, roomTitle: string) => {
    ...
    try {
      ...
      setTotalRooms(totalRooms - 1); // Update total rooms count
  ...
  // Handle navigation to a room details page (for search icon)
          ...
          <div className="room-grid">
            {rooms.map((room, index) => (
              <motion.div
                ...
                custom={index}
              >
                {/* Add premium star for premium rooms */}
                {room.premium && (
                  <div className="featured-star">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                ...
                {/* Update room title section to include lock/unlock and user-specific icons */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <h3 className="room-title text-sm sm:text-base md:text-lg">{room.title}</h3>
                    {/* Add lock/unlock icon based on room type */}
                    {room.type === 'PUBLIC' ? (
                  ...
                  {/* Add search and delete icons for rooms created by the user */}
                  {user && room.createdBy === user._id && (
                    <div className="flex items-center gap-2">
                      {/* Search icon to view room details */}
                      <motion.button
                      ...
                      {/* Delete icon to remove the room */}
                      <motion.button
